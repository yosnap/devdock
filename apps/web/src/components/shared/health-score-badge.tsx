/// Health score circular badge — green (≥80), yellow (50-79), red (<50).
/// Copied from desktop and stripped of Tauri deps.
    ...
    return (
      <Tooltip title="Health score not calculated yet">
        <span style={{
          ...
          fontSize: size === 'small' ? 9 : 11, color: 'var(--icon-muted)',
        }}>
          ?
        </span>
      </Tooltip>
    ...
    <Tooltip title={`Health: ${safeScore}/100 — ${scoreLabel(safeScore)}`}>
      <span style={{ display: 'inline-flex', position: 'relative', cursor: 'default' }}>
        ...
        <span style={{
          ...
          fontSize: size === 'small' ? 9 : 11, fontWeight: 600, color,
        }}>
          {safeScore}
        </span>
      </span>
